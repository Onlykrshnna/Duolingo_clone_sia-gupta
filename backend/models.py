...
# --- Writing System Tables ---
class WritingSystem(Base):
    __tablename__ = "writing_systems"
    ...
    unlock_after_id: Mapped[Optional[uuid.UUID]] = mapped_column(
        ForeignKey("writing_systems.id", ondelete="SET NULL"), nullable=True
    )

    course: Mapped["Course"] = relationship()
    characters: Mapped[List["Character"]] = relationship(
        back_populates="writing_system", cascade="all, delete-orphan", order_by="Character.order_index"
    )
...
class Character(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    writing_system_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("writing_systems.id", ondelete="CASCADE"), nullable=False
    )
    glyph: Mapped[str] = mapped_column(String, nullable=False)
    romanization: Mapped[str] = mapped_column(String, nullable=False)
    pronunciation: Mapped[str] = mapped_column(String, nullable=False)
    example_glyph: Mapped[str] = mapped_column(String, nullable=False)
    example_meaning: Mapped[str] = mapped_column(String, nullable=False)
    image_emoji: Mapped[str] = mapped_column(String, nullable=False, default="📖")
    order_index: Mapped[int] = mapped_column(Integer, nullable=False)

    writing_system: Mapped["WritingSystem"] = relationship(back_populates="characters")


class CharacterProgress(Base):
    __tablename__ = "character_progress"
    __table_args__ = (UniqueConstraint("user_id", "character_id", name="uq_character_progress"),)
...
class CharacterReview(Base):
    __tablename__ = "character_reviews"
    __table_args__ = (UniqueConstraint("user_id", "character_id", name="uq_character_review"),)

    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    user_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("users.id", ondelete="CASCADE"), nullable=False)
    character_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("characters.id", ondelete="CASCADE"), nullable=False)
    due_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    wrong_streak: Mapped[int] = mapped_column(Integer, default=1)
